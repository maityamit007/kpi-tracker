...
function AssetCard({ asset, isTrending = false, handleClick }: { asset: Asset, isTrending?: boolean, handleClick: (event?: React.MouseEvent<HTMLButtonElement>) => void }) {
  return (
    <div onClick={() => handleClick(asset?.chartData ?? [...asset.modalData, {value: asset.hashtags , title: 'hashtags'}] )} className={`flex gap-7 p-4 ${!isTrending ? 'bg-card border rounded-lg shadow-sm' : ''} w-full max-w-sm`}>
